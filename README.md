...
2-ой управляющий слой (на будущее)
----------------------------------

Обычно на клавиатуре принято использовать три слоя (не беря в расчёт национальные раскладки):

* алфавитно-цифровой слой в нижнем регистре
* Shift: слой заглавных букв и доп.символов
* Ctrl: слой управляющих последовательностей
* Alt: слой управляющих последовательностей

В некоторых раскладках добавляют ещё один слой различных символов юникода с вызовом например по RAlt
— см. ru(ruu) или ru(typo2). Не очень удобно назначение разных слоёв разным Alt (или Win/Menu),
поскольку хочется сохранять симметрию. Я в раскладках стремлюсь переопределить модификаторы слева и
справа от пробела, так, чтобы их назначения были симметричны.

Собственно, идея в том, чтобы добавить подобным образом ещё один слой управления, подобный слою с Ctrl.
Но вынести в него на места букв последовательности для навигации — стрелки, home/end и т.п. Удобно сделать
две симметричные группы для навигации в левой и правой части клавиатуры под каждую руку. Вызов слоя
организовать по пробелу, поскольку это самая легко доступная клавиша (сейчас на неё использую маппинг Ctrl).
При этом пробел остаётся доступен при одиночном нажатии, а управляющий слой доступен при удерживании
пробела вместе с целевой клавишей (поведение достигается утилитами xcape или space2ctl).
